    ...
    if current_state==STATE_START :
        ...
        if ch in SIGN:
            if ch in DOUBLE_DELIMITER_FIRST:
                ...
                result.append({"token":ch,"description":"'"+ch+"'","line":line_num})
                tempchar.clear()
                return STATE_START
        if ch.isspace():
            tempchar.clear()
...
def change_id_state(line,index,line_num,current_state):
    ...
    if line[index].isalnum() or line[index]=='_':
        ...
        if ''.join(tempchar).strip() in KEYWORD:
            result.append({"token":''.join(tempchar),"description":''.join(tempchar).upper(),"line":line_num})
            r=True
            for k  in keyword:
            ...
            result.append({"token":''.join(tempchar),"description":"IDENTIFIER","line":line_num})
            r=True
            for k  in ID:
                if k["token"]==''.join(tempchar):
                    r=False
...
def change_literal_state(line,index,line_num,current_state):
    ...
    if current_state==STATE_LITERAL_1:
        ...
        if ch=='"':
            tempchar.append('"')
            result.append({"token":''.join(tempchar),"description":"STRING_LITERAL","line":line_num})
...
def change_digit_state(line,index,line_num,current_state):
    ...
    if current_state==STATE_DIGIT_1:
        ...
        if ch.isspace() or ch==";":
            # tempchar.append(ch)
            result.append({"token":''.join(tempchar),"description":"CONSTANT","line":line_num})
...
def change_note_state(line,index,line_num,current_state):
    ...
    if current_state==STATE_NOTE_1:
        if ch=='*':
            ...
            result.append({"token":"/","description":"'/'","line":line_num})
...
def change_sign_state(line ,index,line_num,current_state):
    ...
    if current_state==STATE_SIGN_2:
        if ch=="=":
            tempchar.append("=")
            if ''.join(tempchar)==">=":
                result.append({"token":''.join(tempchar),"description":"GE_OP","line":line_num})
            elif ''.join(tempchar)=="<=":
                result.append({"token":''.join(tempchar),"description":"LE_OP","line":line_num})
            else:
                result.append({"token":''.join(tempchar),"description":"NE_OP","line":line_num})
            tempchar.clear()
            return STATE_SIGN_4
        else:

            result.append({"token":''.join(tempchar),"description":"'"+''.join(tempchar)+"'","line":line_num})
            tempchar.clear()
            return refresh(ch,line_num)
    ...
    if current_state==STATE_SIGN_3:
        if (ch=="+"and line[index-1]=='+')or (ch=="-"and line[index-1]=='-'):
            tempchar.append(ch)
            if ''.join(tempchar)=="++":
                result.append({"token":''.join(tempchar),"description":"INC_OP","line":line_num})
            else:
                result.append({"token":''.join(tempchar),"description":"DEC_OP","line":line_num})
            tempchar.clear()
            return STATE_SIGN_5
        else:
            result.append({"token":''.join(tempchar),"description":"'"+''.join(tempchar)+"'","line":line_num})
...
def main():
    ...
    file_name="/Users/user/Desktop/b.txt"
